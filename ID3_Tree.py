...
def visualize_tree(tree, parent=None, graph=None):
    ...
    for node in tree:
        if isinstance(node, tuple):
            # If the node is a tuple, it has a label and child nodes
            label, children = node
            graph.node(label)
            if parent is not None:
                graph.edge(parent, label)
            visualize_tree(children, parent=label, graph=graph)
        elif isinstance(node, str):
            # If the node is a string, it's a leaf node (class label)
            graph.node(node)
            if parent is not None:
...
graph = visualize_tree(dictionary_tree)
graph.render('decision_tree')  # Save the tree as 'decision_tree.png'
graph.view('decision_tree')    # Open the tree using the default image viewer
